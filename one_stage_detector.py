...
class DetectorBackboneWithFPN(nn.Module):
    ...
    def __init__(self, out_channels: int):
        ...
        self.lateral3 = nn.Conv2d(64,out_channels=out_channels, stride=1, kernel_size=1)
        self.lateral4 = nn.Conv2d(160,out_channels=out_channels, stride=1, kernel_size=1)
        self.lateral5 = nn.Conv2d(400,out_channels=out_channels, stride=1, kernel_size=1)

        self.out3 = nn.Conv2d(out_channels,out_channels=out_channels, stride=1, kernel_size=3, padding=1)
        self.out4 = nn.Conv2d(out_channels,out_channels=out_channels, stride=1, kernel_size=3, padding=1)
        self.out5 = nn.Conv2d(out_channels,out_channels=out_channels, stride=1, kernel_size=3, padding=1)
        ...
        self.fpn_params = nn.ModuleDict({
            ...
            'lateral4': self.lateral4,
            'lateral5': self.lateral5,
            'out3': self.out3,
            'out4': self.out4,
            'out5': self.out5
        })
    ...
    def forward(self, images: torch.Tensor):
        ...
        l5 = self.relu(self.fpn_params['lateral5'](backbone_feats["c5"]))
        l4 = self.relu(self.fpn_params['lateral4'](backbone_feats["c4"]))
        l3 = self.relu(self.fpn_params['lateral3'](backbone_feats["c3"]))

        i5 = self.relu(self.fpn_params['out5'](l5))
        i4 = self.relu(self.fpn_params['out4'](l4 + F.interpolate(i5, scale_factor = 2, mode='nearest')))
        i3 = self.relu(self.fpn_params['out3'](l3 + F.interpolate(i4, scale_factor = 2, mode= 'nearest')))
...
class FCOSPredictionNetwork(nn.Module):
    ...
    def __init__(
        ...
        # at every location in feature map, we shouldn't "lose" any locations.tride=1, padding=1, padding_mode='zeros', kernel_size=3)
        ######################################################################
        # Fill these.
        stem_cls = []
        stem_box = []
        ...
        # pass
        for output_channels in stem_channels:
            self.conv = nn.Conv2d(in_channels= in_channels, out_channels= output_channels, stride=1, padding=1, padding_mode='zeros', kernel_size=3)
            self.relu = nn.ReLU(inplace=True)
            stem_cls.append(self.conv)
            stem_cls.append(self.relu)
            stem_box.append(self.conv)
            stem_box.append(self.relu)
        ...
        self.pred_cls = nn.Conv2d(stem_channels[-1], num_classes, padding=1, kernel_size=3)  # Class prediction conv
        self.pred_box = nn.Conv2d(stem_channels[-1], 4, padding=1, kernel_size=3)  # Box regression conv
        self.pred_ctr = nn.Conv2d(stem_channels[-1], 1, padding=1, kernel_size=3)  # Centerness conv
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        #     feature = feats_per_fpn_level[key]
        #     proxy = feature.permute(0,2,3,1)
        #     reshaped_feature = proxy(proxy.shape[0], proxy.shape[1]*proxy.shape[2], proxy.shape[3])
        #     class_logits['key'] = self.pred_cls(self.stem_cls(reshaped_feature))
        #     boxreg_deltas['key'] = self.pred_box(self.stem_box(reshaped_feature))
        #     centerness_logits['key'] = self.pred_ctr(self.stem_box(reshaped_feature))
        ...
        class_logits["p4"] = self.pred_cls(self.stem_cls(feats_per_fpn_level["p4"])).view(B, -1, C)
        class_logits["p5"] = self.pred_cls(self.stem_cls(feats_per_fpn_level["p5"])).view(B, -1, C)

        boxreg_deltas["p3"] = self.pred_box(self.stem_box(feats_per_fpn_level["p3"])).view(B, -1, 4)
        boxreg_deltas["p4"] = self.pred_box(self.stem_box(feats_per_fpn_level["p4"])).view(B, -1, 4)
        boxreg_deltas["p5"] = self.pred_box(self.stem_box(feats_per_fpn_level["p5"])).view(B, -1, 4)

        centerness_logits["p3"] = self.pred_ctr(self.stem_box(feats_per_fpn_level["p3"])).view(B, -1, 1)
        centerness_logits["p4"] = self.pred_ctr(self.stem_box(feats_per_fpn_level["p4"])).view(B, -1, 1)
        centerness_logits['p5'] = self.pred_ctr(self.stem_box(feats_per_fpn_level["p5"])).view(B, -1, 1)
...
class FCOS(nn.Module):
    ...
    def forward(
        ...
        matched_gt_boxes = []
        # pass
        number_of_gt = gt_boxes.shape[0]
        for i in range(number_of_gt):
        ...
        for i in range(images.shape[0]):
            ...
            for level, boxes in matched_gt_boxes[i].items():
                ...
                else:  ## to handle the case when there are no matched boxes
        ...
        # Combine predictions and GT matched_gt_boxesfrom across all FPN levels.
        ...
        gt_classes = torch.zeros(matched_gt_boxes.shape[0],matched_gt_boxes.shape[1],self.num_classes,).to(images.device)
        cls_idx = matched_gt_boxes[:, :, -1].to(torch.int64)
        imgs, locs = torch.where((cls_idx != -1))

        for img, loc in zip(imgs, locs):
            gt_classes[img, loc, cls_idx[img, loc]] = 1
    ...
    def inference(
        ...
        for level_name in locations_per_fpn_level.keys():
            ...
            # Assuming level_pred_scores is a tensor
            # threshold_cross = torch.where(level_pred_scores > test_score_thresh, torch.tensor(1), torch.tensor(0)).cpu()
