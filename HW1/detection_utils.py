...
def fcos_get_deltas_from_locations(
    ...
    #     if box[0] == -1:
    #         delta_i = torch.full((1,4), -1)
    #         deltas_list.append(delta_i)
    #         continue

    #     else:
    #         l = locations[i,0] - box[0]
    #         t = locations[i,1] - box[1]
    #         r = box[2] - locations[i,0]
    #         b = box[3] - locations[i,0]
    #         delta_j.append((l, t, r, b))
    #         delta_j = delta_j/stride
    #         deltas_list.append(delta_j)

    # deltas = torch.cat(deltas_list, dim = 0)
    gt_boxes = gt_boxes.to("cuda:0")
    locations = locations.to("cuda:0")
    deltas = torch.zeros([locations.shape[0], 4]).to("cuda:0")
    for i in range(gt_boxes.shape[0]):
        deltas[i, 0] = locations[i, 0] - gt_boxes[i, 0]  
        deltas[i, 1] = locations[i, 1] - gt_boxes[i, 1]  
        deltas[i, 2] = gt_boxes[i, 2] - locations[i, 0]  
        deltas[i, 3] = gt_boxes[i, 3] - locations[i, 1]  
...
def fcos_apply_deltas_to_locations(
    ...
    #                             END OF YOUR CODE                           #
    ##########################################################################
    output_boxes = output_boxes.to("cuda:0")
...
def fcos_make_centerness_targets(deltas: torch.Tensor):
    ...
    centerness = torch.zeros([deltas.shape[0], ]).to("cuda:0")
    l = torch.abs(deltas[:, 0]).to("cuda:0")
    t = torch.abs(deltas[:, 1]).to("cuda:0")
    r = torch.abs(deltas[:, 2]).to("cuda:0")
    b = torch.abs(deltas[:, 3]).to("cuda:0")
...
def get_fpn_location_coords(
    ...
    for level_name, feat_shape in shape_per_fpn_level.items():
        ...
        center_x = x * level_stride + level_stride//2
        center_y = y * level_stride + level_stride//2

        coords = torch.stack((center_x.view(-1), center_y.view(-1)), dim=1)
