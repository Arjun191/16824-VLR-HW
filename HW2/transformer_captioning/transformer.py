...
class AttentionLayer(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        dot_product = torch.matmul(query, key.transpose(-2, -1)) / torch.sqrt(torch.tensor(self.embed_dim, dtype=torch.float32))
        ...
        if attn_mask is not None:
            ...
            additive_mask = dot_product.masked_fill(attn_mask == 0, -1e9)
            dot_product += additive_mask
        
        # apply softmax, dropout, and use value
        attention_weights = F.softmax(dot_product, dim=-1)
        out = self.dropout(attention_weights)
        y =torch.matmul(out, value)
...
class MultiHeadAttentionLayer(AttentionLayer):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        query = self.head_proj(query).view(N, S, H, self.head_dim).transpose(1, 2)  # (N, H, S, head_dim)
        key = self.head_proj(key).view(N, T, H, self.head_dim).transpose(1, 2)  # (N, H, T, head_dim)
        value = self.head_proj(value).view(N, T, H, self.head_dim).transpose(1, 2)  # (N, H, T, head_dim)
...
class PositionalEncoding(nn.Module):
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        self.encoding = nn.Embedding(max_len, embedding_dim=embed_dim) 
        self.dropout = nn.Dropout(dropout)
    ...
    def forward(self, x):
        ...
        pos = torch.arange(S).unsqueeze(0).expand(N, S).to("cuda")
...
class SelfAttentionBlock(nn.Module):
    ...
    def forward(self, seq, mask):
        ...
        dropout = self.dropout(attn_out)
        residual_out = seq + dropout
        out = self.layernorm(residual_out)
...
class CrossAttentionBlock(nn.Module):
    ...
    def forward(self, seq, cond):
        ...
        cross_attn_out = self.cross_attn(seq, cond, cond)
        dropout = self.dropout(cross_attn_out)
        residual_out = seq + dropout
        out = self.norm(residual_out)
